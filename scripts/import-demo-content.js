...
// Sanity-Client importieren
const { createClient } = require('@sanity/client');
const fs = require('fs');
const path = require('path');

// Konfiguration für den Sanity-Client
// Diese Werte müssen an dein Projekt angepasst werden
const client = createClient({
  projectId: 'dein-projekt-id', // Ersetze mit deiner Projekt-ID
  dataset: 'production', // oder 'development', je nach Bedarf
  token: 'dein-token', // Ersetze mit deinem Token (mit Schreibrechten)
  apiVersion: '2023-05-03', // Verwende das aktuelle Datum
  useCdn: false
});
...
// Pfade zu den Demo-Bildern
// Diese Bilder sollten im Ordner 'demo-images' liegen
const demoImagePaths = [
  path.join(__dirname, 'demo-images', 'image1.jpg'),
  path.join(__dirname, 'demo-images', 'image2.jpg'),
  path.join(__dirname, 'demo-images', 'image3.jpg'),
  path.join(__dirname, 'demo-images', 'image4.jpg'),
  path.join(__dirname, 'demo-images', 'image5.jpg')
];
...
async function generateDemoContent() {
  ...
  try {
    // Erstelle den Ordner für Demo-Bilder, falls er nicht existiert
    const demoImagesDir = path.join(__dirname, 'demo-images');
    if (!fs.existsSync(demoImagesDir)) {
      console.log('Demo-Bilder-Ordner existiert nicht. Bitte erstelle ihn und füge Bilder hinzu.');
      console.log(`Erwarteter Pfad: ${demoImagesDir}`);
      return;
    }
    
    // Überprüfe, ob Demo-Bilder vorhanden sind
    const demoImages = fs.readdirSync(demoImagesDir).filter(file => 
      file.endsWith('.jpg') || file.endsWith('.jpeg') || file.endsWith('.png')
    );
    
    if (demoImages.length === 0) {
      console.log('Keine Demo-Bilder gefunden. Bitte füge Bilder zum Ordner hinzu.');
      return;
    }
    
    console.log(`${demoImages.length} Demo-Bilder gefunden.`);
 ...
 * 
 * 2. Erstelle einen Ordner 'demo-images' im selben Verzeichnis wie dieses Skript
 *    und füge dort einige Bilder hinzu (JPG, JPEG oder PNG).
 * 
 * 3. Aktualisiere die Sanity-Client-Konfiguration mit deinen Projekt-Daten:
 *    - projectId: Deine Sanity-Projekt-ID
 *    - dataset: Dein Sanity-Dataset (normalerweise 'production')
 *    - token: Ein Sanity-Token mit Schreibrechten
 * 
 * 4. Führe das Skript aus:
 *    node import-demo-content.js
 */
